...
PlayStation®TV ¯\_(ツ)_/¯ Full Version
*/
...
service.create(plugin.title, plugin.id + ":start", 'By DL CINE PLAY TV™', true, logo);
...
function showPlaylist(page) {
    ...
    if (!playlist || !playlist.toString()) 
        popup.notify('Alimentada By DL CINE PLAY TV™', 5);
...
function readAndParseM3U(page, pl, m3u) {
    ...
    page.loading = true;
    if (!m3u) {
        page.metadata.title = 'By DL CINE PLAY TV™...';
...
new page.Route(plugin.id + ":loadOnePlaylist", function(page) {
    setPageHeader(page, 'FILMES DL PLAY');
    page.loading = true;
    ...
    page.metadata.title =  'FILMES DL PLAY (' + showM3U(page) + ')';
    page.loading = false;
});
...
new page.Route(plugin.id + ":start", function(page) {
    ...
    page.appendItem("", "separator", {
        title: '.:: Copyright © 2024 – DL PLAY TV™ | Todos os direitos reservados. ::.'
    });
   page.appendItem("", "separator", {
        title: ' '
    });
	...
	if (!service.disableSampleXMLList) {
			page.model.contents = 'grid';
        var item = page.appendItem('m3u:http://smarterscdn.xyz:80/get.php?username=dlgames3&password=31355431&type=m3u_plus&output=ts:IPTV', "directory", {
            title: ' FILMES',
			icon: 'https://raw.githubusercontent.com/DLGAMESHEN/dlcinetv/main/IPTV_DLPLAY/LOGO_MENU/1.png'
        });
    }
	
	if (!service.disableSampleXMLList) {
			page.model.contents = 'grid';
        var item = page.appendItem('m3u:http://smarterscdn.xyz:80/get.php?username=dlgames4&password=313415341&type=m3u_plus&output=ts:IPTV', "directory", {
            title: 'SÉRIES',
			icon: 'https://raw.githubusercontent.com/DLGAMESHEN/dlcinetv/main/IPTV_DLPLAY/LOGO_MENU/2.png'
        });
    }
    ...
    if (!service.disableSampleList) {
		...
		var item = page.appendItem('m3u:http://smarterscdn.xyz:80/get.php?username=dlgames3&password=31355431&type=m3u_plus&output=ts:IPTV', "directory", {
            title: 'DESENHOS',
			icon: 'https://raw.githubusercontent.com/DLGAMESHEN/dlcinetv/main/IPTV_DLPLAY/LOGO_MENU/3.png'
        });
    }

    if (!service.disableSampleXMLList) {
			page.model.contents = 'grid';
        var item = page.appendItem('m3u:https://dlplaytv.danielstorevip.workers.dev/0:/LISTA%20M3U/animes_iptv.m3u:IPTV', "directory", {
            title: 'ANIMES',
			icon: 'https://raw.githubusercontent.com/DLGAMESHEN/dlcinetv/main/IPTV_DLPLAY/LOGO_MENU/4.png'
        });
    }
	if (!service.disableSampleXMLList) {
	 	page.model.contents = 'grid';
        var item = page.appendItem('m3u:http://smarterscdn.xyz:80/get.php?username=dlgames&password=426355263&type=m3u_plus&output=ts:IPTV', "directory", {
            title: 'CANAIS',
			icon: 'https://raw.githubusercontent.com/DLGAMESHEN/dlcinetv/main/IPTV_DLPLAY/LOGO_MENU/5.png'
        });
    }    
